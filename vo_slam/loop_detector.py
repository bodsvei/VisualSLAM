...
class LoopDetector:
    ...
    def _geometric_verify(
        ...
        Verify loop geometrically via:
          ...
          2. PnP RANSAC using 3D points from match_kf
        ...
        # Resolve 3D-2D correspondences
        pts_q_2d = []
        pts_m_3d = []
        
        for m in good:
            mp = match_kf.get_map_point_at_feature(m.trainIdx)
            if mp is not None:
                pts_q_2d.append(q_pts[m.queryIdx])
                pts_m_3d.append(mp.xyz)

        if len(pts_q_2d) < self.min_geo_inliers:
            return None

        pts_q_2d = np.array(pts_q_2d, dtype=np.float32)
        pts_m_3d = np.array(pts_m_3d, dtype=np.float32)

        # Solve PnP with RANSAC
        # Since pts_m_3d are in World frame, solvePnP gives T_query_world
        success, rvec, tvec, inliers = cv2.solvePnPRansac(
            pts_m_3d, pts_q_2d, self.camera_K, None,
            iterationsCount=100,
            reprojectionError=5.0,
            confidence=0.99
        )
        
        if not success or inliers is None or len(inliers) < self.min_geo_inliers:
            return None

        # T_q_w is World -> QueryCam
        R_q_w, _ = cv2.Rodrigues(rvec)
        T_q_w = np.eye(4)
        T_q_w[:3, :3] = R_q_w
        T_q_w[:3, 3]  = tvec.ravel()
        
        # We need T_rel = match_cam -> query_cam = T_query_world @ T_world_match
        # match_kf.T_world_cam is match_cam -> World
        T_rel = T_q_w @ match_kf.T_world_cam

        return LoopEvent(
            query_kf_id = candidate.query_kf_id,
            match_kf_id = candidate.match_kf_id,
            bow_score   = candidate.score,
            geo_inliers = len(inliers),
            T_rel       = T_rel,
        )
