...
# 1. Fix _recover_scale
recover_scale_code = """    def _recover_scale(self, kf: Keyframe, match_kf, R: np.ndarray, t: np.ndarray) -> float:
        \"\"\"
        ...
        ratio = np.median(filtered_ratios) if len(filtered_ratios) >= 3 else np.median(ratios)
        return float(np.clip(ratio, self.cfg.scale_clamp_min, self.cfg.scale_clamp_max))"""

code = re.sub(r'    def _recover_scale\(self, kf: Keyframe, match_kf, R: np.ndarray, t: np.ndarray\) -> float:.*?return float\(np.clip\(ratio, self.cfg.scale_clamp_min, self.cfg.scale_clamp_max\)\)', recover_scale_code, code, flags=re.DOTALL)
